...
class Utils {
    constructor() {
        ...
        /**
         * Checks whether the current user exist or not.
         * @param {*} token JWT Token of the User.
         */
        this.verifyUser = (token) => {
            //try{
            //  console.log("in verifyuser", token);
            let decodedToken = jwt.verify(token, config.jwttokenkey);
            //console.log("decoded token", decodedToken);
            return User.getUserById(decodedToken.uid).then((data) => {
                if (!_.isEmpty(data)) return decodedToken;

                return {
                    status: config.failed,
                    message: config.errors.userNotExist,
                };
            });
            // }catch(err){
            //     console.log('verifyUser..', err);
            //     return Promise.resolve({ status: config.failed, message: config.errors.invalidToken});
            // }
        };
        /**
        ...
        this.handleResponse = (invoice, response) => {
            ...
            if (response) {
                ...
                let isObjectData =
                    typeof response.data === "object" ? true : false;
                if (!isObjectData) {
                    data = {
        ...
        this.formatInvoiceNo = (input, length) => {
            if (input) {
                let prefix = "W";
                if (input.toString().length >= length) return prefix + input;

                return (
                    prefix +
                    Array(length - Math.floor(Math.log10(input))).join("0") +
                    input
                );
            }
